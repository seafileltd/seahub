...
def generate_file_lock_key_value(request):
    ...
    request_user, repo_id, file_path, obj_id = get_file_info_by_token(token)
...
class WOPIFilesView(APIView):
    ...
    def get(self, request, file_id, format=None):
        ...
        if ENABLE_OFFICE_WEB_APP_EDIT and not repo.encrypted and \
                ...
                (is_locked and locked_by_online_office)) and \
                request_user != ANONYMOUS_EMAIL:
...
class WOPIFilesContentsView(APIView):
    ...
    def get(self, request, file_id, format=None):
        ...
        token = request.GET.get('access_token', None)
        request_user, repo_id, file_path, obj_id = get_file_info_by_token(token=token)
